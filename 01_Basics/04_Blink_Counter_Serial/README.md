...
Όταν ανεβάσετε το firmware στο Arduino Uno και ανοίξετε τον Serial Monitor:
1. Στο setup(), βλέπετε "Arduino Starting Up...".
2. Στο πρώτο πέρασμα του loop():
    - Η static int flashCount = 0; εκτελείται μία φορά.
    - Το flashCount γίνεται 1.
    - Το LED αναβοσβήνει.
    - Στον Serial Monitor βλέπετε: "Το LED αναβόσβησε: 1 φορές."
3. Στο δεύτερο πέρασμα του loop():
    - Η γραμμή static int flashCount = 0; αγνοείται (η τιμή έχει ήδη οριστεί).
    - Το flashCount γίνεται 2.
    - Το LED αναβοσβήνει.
    - Στον Serial Monitor βλέπετε: "Το LED αναβόσβησε: 2 φορές."
    - ... Και ούτω καθεξής.
...
...
```
Χωρίς τη λέξη-κλειδί `static`, ο μετρητής flashCount θα μηδενιζόταν σε 0 στην αρχή κάθε επανάληψης του loop (ακριβώς όπως μια κανονική τοπική μεταβλητή), και θα βλέπατε πάντα το μήνυμα "Το LED αναβόσβησε: 1 φορές."
