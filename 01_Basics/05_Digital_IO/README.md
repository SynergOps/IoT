...
*   **Αισθητήρες Πόρτας/Παραθύρου:** Μαγνητικές επαφές (Reed switches) σε συστήματα συναγερμού λειτουργούν ακριβώς όπως ένα κουμπί (ανοιχτό/κλειστό).
*   **Emergency Stop:** Σε βιομηχανικό IoT (IIoT), κουμπιά έκτακτης ανάγκης διακόπτουν άμεσα τη λειτουργία μηχανημάτων.
*   **User Interface:** Κουμπιά "Reset" ή "WPS" σε routers και smart home hubs για επαναφορά ρυθμίσεων ή σύνδεση.
...
Το **breadboard** (ή **prototyping board**) είναι ένα εργαλείο που χρησιμοποιούμε για να συνδέουμε ηλεκτρονικά εξαρτήματα **χωρίς να χρειάζεται να τα κολλάμε** (soldering).
...
Όψη από πάνω:
┌──────────────────────────────────────────────┐
│  [─]  ←── Ράγες τροφοδοσίας (Power Rails)    │
│  [+]      (όλα συνδεδεμένα κάθετα)           │
│                                              │
│      a   b   c   d   e       f   g   h   i   j
│  1   •   •   •   •   •   |   •   •   •   •   •
│  2   •   •   •   •   •   |   •   •   •   •   •
...
Κάθετη τομή (πώς συνδέονται μέσα):
        a  b  c  d  e     f  g  h  i  j
     1  •  •  •  •  •     •  •  •  •  •
        └──┴──┴──┴──┘     └──┴──┴──┴──┘
         Μεταλλική          Μεταλλική
         ράβδος             ράβδος
       (5 τρύπες μαζί)   (5 τρύπες μαζί)
```

**Βασικοί Κανόνες:**
- **Οριζόντια σύνδεση (ίδια σειρά):** Οι οπές a-b-c-d-e της ίδιας σειράς είναι συνδεδεμένες
- **Χωρισμός στη μέση:** Οι δύο πλευρές (a-e και f-j) ΔΕΝ συνδέονται μεταξύ τους
- **Ράγες τροφοδοσίας:** Οι στήλες + και - στα πλάγια συνδέονται κάθετα (για VCC και GND)
...
Αν συνδέσεις ένα καλώδιο στο [1a]
και άλλο καλώδιο στο [1d]:
   ✅ Θα είναι συνδεδεμένα (ίδια σειρά = ίδιο κύκλωμα)

Αν συνδέσεις καλώδιο στο [1b]
και άλλο στο [2b]:
   ❌ ΔΕΝ θα είναι συνδεδεμένα (διαφορετική σειρά)
```

#### Πλεονεκτήματα:

✅ **Ευέλικτο** - Μπορείς να αλλάξεις το κύκλωμα ή να προσθέσεις νέα εξαρτήματα εύκολα  
✅ **Γρήγορο** - Δεν χρειάζεται κόλληση (soldering)  
✅ **Φιλικό για αρχάριους** - Τέλειο για μάθηση και δοκιμές  
✅ **Επαναχρησιμοποιήσιμο** - Μπορείς να χρησιμοποιήσεις το ίδιο breadboard για πολλά projects
...
- **`pinMode(pin, INPUT_PULLUP)`**: Αυτή είναι μια παραλλαγή του `pinMode`. Όταν ορίζουμε μια ακίδα ως `INPUT`, θέλουμε να διαβάσουμε την τάση της. Ωστόσο, αν η ακίδα δεν είναι συνδεδεμένη πουθενά, η τιμή της "αιωρείται" (floating) και μπορεί να διαβάζει τυχαία `HIGH` ή `LOW`.
    - Το `INPUT_PULLUP` ενεργοποιεί μια εσωτερική "αντίσταση ανύψωσης" (pull-up resistor) μέσα στο τσιπ του Arduino. Αυτή η αντίσταση συνδέει την ακίδα με την τροφοδοσία (5V), οπότε από προεπιλογή η ακίδα διαβάζει `HIGH`. Όταν πατήσουμε το κουμπί και συνδέσουμε την ακίδα με τη γείωση (GND), η τιμή της γίνεται `LOW`.
    - **Με απλά λόγια:** Χρησιμοποιώντας το `INPUT_PULLUP`, η ακίδα είναι `HIGH` όταν το κουμπί **δεν** είναι πατημένο, και `LOW` όταν **είναι** πατημένο. Αυτό μας γλιτώνει από το να προσθέτουμε εξωτερική αντίσταση στο κύκλωμά μας.

- **`digitalRead(pin)`**: Αυτή η συνάρτηση διαβάζει την κατάσταση μιας ψηφιακής ακίδας. Επιστρέφει `HIGH` (συνήθως 5V) ή `LOW` (0V).

- **`if (condition) { ... }`**: Ο πιο βασικός τρόπος για να παίρνουμε αποφάσεις στον κώδικα. Αν η `condition` μέσα στην παρένθεση είναι αληθής, τότε εκτελείται ο κώδικας μέσα στις αγκύλες `{}`.
...
## Επόμενο Βήμα
Μάθετε **06_Millis_Timing** για μη-αποκλειστικό χρονισμό χωρίς `delay()`.
